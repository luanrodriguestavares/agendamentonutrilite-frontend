...
const normalizeDate = (dateInput) => {
	...
	try {
		...
		if (typeof dateInput === "string" && /^\d{4}-\d{2}-\d{2}$/.test(dateInput)) {
			...
			console.log("normalizeDate - string YYYY-MM-DD:", { input: dateInput, result: date.toISOString().split("T")[0] })
		} else {
			date = new Date(dateInput)
			if (typeof dateInput === "string" && dateInput.includes('T') && dateInput.includes('Z')) {
				const localDate = new Date(date.getTime() + date.getTimezoneOffset() * 60000)
				date = new Date(localDate.getFullYear(), localDate.getMonth(), localDate.getDate(), 0, 0, 0, 0)
				console.log("normalizeDate - UTC string:", { input: dateInput, result: date.toISOString().split("T")[0] })
			}
		}
...
const getAgendamentoQuantidadeTotal = (agendamento) => {
	switch (agendamento.tipoAgendamento) {
		case "Coffee Break":
			return Number(agendamento.quantidade) || 0
		...
		case "Home Office":
			...
			if (typeof agendamento.refeicoes === "string" && agendamento.refeicoes.startsWith("[")) {
				try {
					const arr = JSON.parse(agendamento.refeicoes)
					return Array.isArray(arr) ? arr.length : 1
				} catch {
					return 1
				}
			}
			return agendamento.refeicoes ? 1 : 0
		case "Administrativo - Lanche":
			...
			return 1
		case "Rota Extra":
			return (Number(agendamento.quantidadeTiangua) || 0) + (Number(agendamento.quantidadeUbajara) || 0)
		default:
			return 0
	}
}
...
const getMainAgendamentoDate = (agendamento) => {
	try {
		...
		switch (agendamento.tipoAgendamento) {
			case "Home Office":
				result = normalizeDate(agendamento.dataInicio)
				break
			case "Agendamento para Time":
				result = normalizeDate(agendamento.dataFeriado || agendamento.dataInicio)
				break
			case "Administrativo - Lanche":
			case "Agendamento para Visitante":
				result = normalizeDate(agendamento.data)
				break
			case "Coffee Break":
				result = normalizeDate(agendamento.dataCoffee)
				break
			case "Rota Extra":
				result = normalizeDate(agendamento.dataInicio)
				break
			default:
				result = normalizeDate(agendamento.createdAt)
		}

		if (agendamento.id === "dc1528df-e649-4ff3-a1f9-c845a8ad7c4d") {
			console.log("Debug agendamento 04/07:", {
				id: agendamento.id,
				tipo: agendamento.tipoAgendamento,
				dataInicio: agendamento.dataInicio,
				dataFim: agendamento.dataFim,
				result: result ? result.toISOString().split("T")[0] : null
			})
		}
...
function getDashboardStats(filteredAgendamentos, filters) {
	...
	const totalRefeicoes = ags.reduce((acc, ag) => acc + getAgendamentoQuantidadeTotal(ag), 0)
	const totalTransportes = ags.reduce((acc, ag) => {
		if (ag.tipoAgendamento === "Rota Extra") {
			return acc + getAgendamentoQuantidadeTotal(ag)
		}
		return acc
	}, 0)
	...
	const agsHoje = ags.filter((ag) => {
		...
		if (d) {
			const adjustedAgendamentoDate = new Date(d.getTime() + d.getTimezoneOffset() * 60000)
			const agendamentoDay = new Date(adjustedAgendamentoDate.getFullYear(), adjustedAgendamentoDate.getMonth(), adjustedAgendamentoDate.getDate())
			const todayDay = new Date(adjustedToday.getFullYear(), adjustedToday.getMonth(), adjustedToday.getDate())
			return agendamentoDay.getTime() === todayDay.getTime()
		}
		return false
	})
	const totalAgsHoje = agsHoje.length
	const totalRefeicoesHoje = agsHoje.reduce((acc, ag) => acc + getAgendamentoQuantidadeTotal(ag), 0)
	const totalTransportesHoje = agsHoje.reduce((acc, ag) => {
		if (ag.tipoAgendamento === "Rota Extra") {
			return acc + getAgendamentoQuantidadeTotal(ag)
		}
		return acc
...
function hasAdvancedFilters(filters) {
	return Object.entries(filters).some(([key, value]) => {
		if (key === "tipoAgendamento") return false
...
function isSameOrAfterDay(dateA, dateB) {
	return (
...
function isSameOrBeforeDay(dateA, dateB) {
	return (
		dateA.getFullYear() < dateB.getFullYear() ||
		(dateA.getFullYear() === dateB.getFullYear() && dateA.getMonth() < dateB.getMonth()) ||
		(dateA.getFullYear() === dateB.getFullYear() && dateA.getMonth() === dateB.getMonth() && dateA.getDate() <= dateB.getDate())
...
export default function Admin() {
	...
	const isAgendamentoAtivo = (agendamento) => {
		try {
			...
			const adjustedToday = new Date(today.getTime() + today.getTimezoneOffset() * 60000)
			const adjustedAgendamentoDate = new Date(agendamentoDate.getTime() + agendamentoDate.getTimezoneOffset() * 60000)
			const agendamentoDay = new Date(adjustedAgendamentoDate.getFullYear(), adjustedAgendamentoDate.getMonth(), adjustedAgendamentoDate.getDate())
			const todayDay = new Date(adjustedToday.getFullYear(), adjustedToday.getMonth(), adjustedToday.getDate())
			return isAfter(agendamentoDay, todayDay) || agendamentoDay.getTime() === todayDay.getTime()
		} catch (error) {
			return false
		}
	}
		...
		Object.keys(filters).forEach((key) => {
			...
			if (value && value !== "" && key !== "periodoInicio" && key !== "periodoFim") {
				if (key === "tiposAgendamento") {
					...
					filtered = filtered.filter((agendamento) => {
						const quantidade = getAgendamentoQuantidadeTotal(agendamento)
	...
	const exportToExcel = (dataToExport = null) => {
		...
		const formatDateForExcel = (dateString) => {
			...
			try {
				...
				return adjustedDate ? format(adjustedDate, "dd/MM/yyyy") : ""
			} catch {
				return ""
			}
		}
		...
		const formatDateTimeForExcel = (dateString) => {
			...
			try {
				const adjustedDate = adjustDate(dateString)
				return adjustedDate ? format(adjustedDate, "dd/MM/yyyy HH:mm") : ""
			} catch {
				return ""
			}
		}
		...
		const getExportColumns = () => {
			...
			if (tiposSelecionados.length > 0) {
				if (tiposSelecionados.length === 1 && typeSpecificCols[tiposSelecionados[0]]) {
					return typeSpecificCols[tiposSelecionados[0]]
				} else {
					...
					return Array.from(allRelevantCols)
				}
			} else if (tipoUnico && typeSpecificCols[tipoUnico]) {
				return typeSpecificCols[tipoUnico]
			} else {
				const allRelevantCols = new Set()

				baseCols.forEach((col) => allRelevantCols.add(col))

				Object.values(typeSpecificCols).forEach((cols) => {
					cols.forEach((col) => allRelevantCols.add(col))
				})
				...
				return gruposOrganizados.filter((col) => allRelevantCols.has(col))
			}
		}
		...
		const columnMapping = {
			...
			status: "Status",
			timeSetor: "Time/Setor",
			centroCusto: "Centro de Custo",
			turno: "Turno",
			refeitorio: "Refeitório",
			dataInicio: "Data Início",
			dataFim: "Data Fim",
			dataCoffee: "Data Coffee",
			data: "Data",
			...
			quantidade: "Qtd Geral",
			localEntrega: "Local Entrega",
			rateio: "Rateio",
			acompanhante: "Acompanhante",
			nomeVisitante: "Nome Visitante",
			refeicoes: "Refeições",
			isFeriado: "É Feriado",
			dia: "Categoria Dia",
		...
		const getColumnValue = (agendamento, column) => {
			switch (column) {
				case "nome":
				...
				case "tipoAgendamento":
					return agendamento.tipoAgendamento || ""
				case "tipoServico":
					return agendamento.tipoServico || ""
				case "status":
					return agendamento.status || ""
				case "timeSetor":
					return agendamento.timeSetor || ""
				case "centroCusto":
					return agendamento.centroCusto || ""
				case "turno":
					return agendamento.turno || ""
				case "refeitorio":
					return agendamento.refeitorio || ""
				case "dataInicio":
					return formatDateForExcel(agendamento.dataInicio)
				case "dataFim":
					return formatDateForExcel(agendamento.dataFim)
				case "dataCoffee":
					return formatDateForExcel(agendamento.dataCoffee)
				case "data":
					return formatDateForExcel(agendamento.data)
				case "dataFeriado":
					return formatDateForExcel(agendamento.dataFeriado)
				case "horario":
				...
				case "quantidadeTiangua":
					return agendamento.quantidadeTiangua || ""
				case "quantidadeUbajara":
					return agendamento.quantidadeUbajara || ""
				case "quantidade":
					return agendamento.quantidade || ""
				case "localEntrega":
					return agendamento.localEntrega || ""
				case "rateio":
					return agendamento.rateio || ""
				case "acompanhante":
					return agendamento.acompanhante || ""
				case "nomeVisitante":
					return agendamento.nomeVisitante || ""
				case "refeicoes":
					return formatRefeicoes(agendamento.refeicoes)
				case "isFeriado":
				...
				case "createdAt":
					return formatDateTimeForExcel(agendamento.createdAt)
				default:
					return ""
			}
		}
		...
		const exportData = data.map((agendamento) => {
			...
			exportColumns.forEach((column) => {
				if (column !== "actions") {
					...
					row[columnName] = getColumnValue(agendamento, column)
				}
			})
			return row
		})
	...
	const formatDate = (dateString) => {
		...
		try {
			const adjustedDate = adjustDate(dateString)
			return adjustedDate ? format(adjustedDate, "dd/MM/yyyy", { locale: ptBR }) : "-"
		} catch {
			return "-"
		}
	}
	...
	const formatDateTime = (dateString) => {
		...
		try {
			const adjustedDate = adjustDate(dateString)
			return adjustedDate ? format(adjustedDate, "dd/MM/yyyy HH:mm", { locale: ptBR }) : "-"
		} catch {
			return "-"
		}
	}
	...
	const allColumns = {
		...
		quantidadePrincipal: {
			...
			minWidth: "100px",
			cell: (ag) => {
				const quantidade = getAgendamentoQuantidadeTotal(ag)
				let detalhes = ""
				...
				const displayQuantidade = quantidade > 0 ? quantidade : "-"

				return (
					<TableCell title={`${detalhes}: ${displayQuantidade}`}>
						<div className="text-center">
							<div className="font-medium">{displayQuantidade}</div>
							{detalhes && displayQuantidade !== "-" && <div className="text-xs text-gray-500">{detalhes}</div>}
						</div>
					</TableCell>
				)
			},
						...
						<Card>
							<CardContent className="p-0">
								...
								{currentAgendamentos.length === 0 ? (
									...
									<>
										<div className="overflow-x-auto scrollbar-thick scrollbar-thumb-gray-400 scrollbar-track-gray-100 hover:scrollbar-thumb-gray-500 rounded-lg">
											<Table>
												<TableHeader>
													<TableRow>
														{visibleColumns.map((colKey) => {
															...
															return (
																<TableHead
																	key={colKey}
																	className={`${col.sortable ? "cursor-pointer" : ""} ${col.sticky ? "sticky right-0 bg-white rounded-r-lg" : ""
																		}`}
				...
				<DialogContent>
					...
					<div className="py-4">
						<Label htmlFor="cancelReason" className="mb-2 block">
							Deseja informar o motivo do cancelamento? (opcional)
						</Label>
						<Textarea
							id="cancelReason"
							value={cancelReason}
							onChange={(e) => setCancelReason(e.target.value)}
							placeholder="O motivo do cancelamento será enviado para o e-mail do solicitante."
